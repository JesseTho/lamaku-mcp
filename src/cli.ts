...
async function cmdDoctor(): Promise<void> {
  ...
  if (!status.authenticated) {
    log('No stored session. Run `lamaku-mcp login`.');
    process.exitCode = 1;
    return;
  }
...
const USAGE = `lamaku-mcp <command>
...
Set LAMAKU_HOST for a school other than UH, e.g.
  LAMAKU_HOST=brightspace.example.edu lamaku-mcp login
`;
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'serve': {
      ...
      // `npx -y lamaku-mcp` with no arguments.
