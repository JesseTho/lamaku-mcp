 ...
 * app registration form. Hand this list to UH ITS verbatim if they agree to
 * register a client.
 */
 ...
 * registers a client and LAMAKU_CLIENT_ID / _SECRET are set — at which
 * point set LAMAKU_AUTH=oauth and nothing else in the server changes.
 */
export class OAuthProvider implements AuthProvider {
  ...
  constructor(
    ...
    private readonly clientId = process.env.LAMAKU_CLIENT_ID,
    private readonly clientSecret = process.env.LAMAKU_CLIENT_SECRET,
  ) {}
    ...
    if (!this.tokens) {
      throw new AuthError(
        'No OAuth tokens stored. Complete the authorization-code flow first, ' +
          'or set LAMAKU_AUTH=session to use browser-session auth instead.',
      );
    }
