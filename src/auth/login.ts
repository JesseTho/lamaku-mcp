 ...
 * Edge means `npx lamaku-mcp` needs no 190 MB browser download — which
...
async function openContext(headless: boolean): Promise<BrowserContext> {
  ...
  const channels = process.env.LAMAKU_BROWSER
    ? [process.env.LAMAKU_BROWSER]
    : BROWSER_CHANNELS;
  ...
  throw new Error(
    'Could not launch a browser to sign in with. Install Google Chrome or ' +
      ...
      'LAMAKU_BROWSER to force a channel (chrome, msedge, chromium).\n\n' +
      `Tried:\n  ${failures.join('\n  ')}`,
  );
}

/**
 * Opens a real browser window and waits for the user to complete UH's
...
export async function interactiveLogin(
  ...
  try {
    ...
    log('');
    log('  A browser window is open. Sign in to Lamaku with your UH');
    log('  username and approve the Duo prompt. This window will close by');
...
/**
 * Re-derive a session without user interaction. Works when UH's IdP still has
