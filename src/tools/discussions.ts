...
import { authorLabel, privacyNote } from '../privacy.js';
    ...
    'read_posts',
    {
      ...
      inputSchema: {
        ...
        revealStudents: z
          ...
          .optional()
          .describe(
            'Return real student names instead of stable pseudonyms. Student ' +
              'names are protected education records, so set this only when the ' +
              'user has explicitly asked to see who wrote what.',
          ),
      },
    },
    async ({ course, forumId, topicId, limit, revealStudents }) =>
      guard(async () => {
        const orgUnitId = await resolveOrgUnitId(client, course);
        const path = await client.le(
            ...
            throw new Error(
              topic?.MustPostToParticipate
                ? `"${topic.Name}" requires you to post before you can read anyone ` +
                  `else's replies, so Lamaku is withholding them. Post first with ` +
                  `create_discussion_post, then read.`
                : `Lamaku returned 403 for the posts in this topic. The usual cause ` +
        ...
        // Discussion threads are the single biggest source of student names in
        // this server's output, so they go through the FERPA guard.
        const authorNames = visible
          .map((p) => p.PostingUserDisplayName)
        ...
        return ok({
          ...
          ...privacyNote(revealStudents ?? false, authorNames.length),
          posts: visible.map((p) => ({
            postId: p.PostId,
            ...
            author: p.IsAnonymous
              ...
              : authorLabel(
                  p.PostingUserId,
                  p.PostingUserDisplayName,
                  revealStudents ?? false,
                ),
            ...
            'reply_to_post',
            {
              ...
              replyingTo: {
                ...
                author: parent.IsAnonymous
                  ? 'Anonymous'
                  : authorLabel(parent.PostingUserId, parent.PostingUserDisplayName),
